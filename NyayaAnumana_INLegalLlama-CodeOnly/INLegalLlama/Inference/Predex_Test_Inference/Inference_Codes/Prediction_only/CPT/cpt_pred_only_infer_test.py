...
df=pd.read_csv("predex_test.csv") ###PROVIDE THE PATH TO THE TEST DATA(DOWNLOADED FROM README OF PARENT DIRECTORY) OVER WHICH INFERENCE SHOULD BE DONE###
...
peft_model_dir = "" ###GIVE THE PATH TO YOUR LOCAL MODEL(CPT MODEL OBTAINED BY EXECUTING CODES IN CONTINUED PRE-TRAINED FOLDER)/HUGGINGFACE PATH FOR THAT MODEL,OVER WHICH INFERENCE NEEDS TO BE EVALUATED###
use_4bit = True
...
with open("output.csv", 'a', newline='', encoding='utf-8') as f: ###SPECIFY PATH TO RESULTING INFERED CSV FILE,WHERE AN EXTRA COLUMN NAMED "LLAMA2_PRED" WILL BE CREATED AS A REPRESENTATIVE OF MODEL PREDICTIONS ###
    ...
    for i, row in tqdm(df.iterrows(), total=df.shape[0]):
        ...
        Analyze the case proceeding and predict whether the appeal/petition will be accepted (1) or rejected (0).Just give 1 or 0 accordingly only\
        ...
        outputs = trained_model.generate(input_ids=input_ids, max_new_tokens=5)
