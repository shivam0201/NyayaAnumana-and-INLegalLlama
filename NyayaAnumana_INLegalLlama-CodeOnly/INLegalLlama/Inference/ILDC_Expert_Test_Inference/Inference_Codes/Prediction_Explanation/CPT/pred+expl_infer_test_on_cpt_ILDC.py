...
df=pd.read_csv("") #provide the path to the test data over which Inference should be done
...
for i, row in tqdm(df.iterrows(), total=df.shape[0]):
    inp = row['Case Description']
    inpu = preprocess_input(inp)
    df.at[i, 'Case Description'] = inpu

# Set up model and tokenizer configurations
peft_model_dir = "" #specify the path to the PEFT model
use_4bit = True
...
with open("", 'a', newline='', encoding='utf-8') as f: #specify path to resulting Infered CSV file,where an extra column named "llama2_pred" will be created as a representative of model predictions
    writer = csv.writer(f)
    writer.writerow(list(df.columns) + ["llama2_pred"])  # Write the header

    for i, row in tqdm(df.iterrows(), total=df.shape[0]):
        case_pro = row['Case Description']
        ...
        outputs = trained_model.generate(input_ids=input_ids, max_new_tokens=800)
