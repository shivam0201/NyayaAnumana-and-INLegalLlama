...
df=pd.read_csv("") ### PROVIDE THE PATH TO "PREDEX_TRAIN.CSV" DOWNLOADED FROM README OF CURRENT DIRECTORY ###
...
# formatting prompt instruction for prediction
def format_instruction_pred(sample, inst):
...
for i,row in df.iterrows():
  ...
  t = format_instruction_pred(df.iloc[i],df_ins['Instructions'][random_index])
  text.append(t)
